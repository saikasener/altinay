...
# --------------------------------------------------------------------------------------------------
    def __init__(self):
        ...
        #/* link layer */
        self.szLinkLayer = "sockraw eno2 1"                   #/* Link layer settings */
        #/* timing */
        self.dwBusCycleTimeUsec = 50          #/* bus cycle time in usec */
...
# --------------------------------------------------------------------------------------------------
    def __init__(self):
        ...
        self.processImageRt = EcProcessImage(self, True)  #/* process image with realtime support (access only  job task) */
        self.oReg = None
...
#                self.myAppDiagnosis()
                time.sleep(.01)
#                print(f'\n------------------- Selected Trajectory-> {TrajSelect} --------------------\n')
#                TrVel += 1
                RMR,LRM = WheelVelocity(self,AgvLinearVel,AgvAngVel,Rdir,Ldir) # (self,LinVel,AngVel,Rdir,Ldir,):

                # print(f'\n----------------- Trajectory Velocity (Rpm) -> {Rdir*RMR} ------------------\n')
                # print(f'------------------- Actual Velocity (Rpm) -> {GetVelActual(self)} --------------\n')
#                if TrVel == 3500:
#                    WheelStop(self)
#                    time.sleep(.001)
#                LiftUp(self,1)
...
# --------------------------------------------------------------------------------------------------
# ---------------------------------- Trajectory Function Area --------------------------------------
# --------------------------------------------------------------------------------------------------
def MotorInit(self):
    self.processImage.variables.RightMotor.RxPDO_Mapping_1.Modes_of_operation.set(3)
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_acceleration.set(4000)
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_deceleration.set(4000)
    ...
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_acceleration.set(4000)
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_deceleration.set(4000)
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
# --------------------------------------------------------------------------------------------------
def WheelVelocity(self,LinVel,AngVel,Rdir,Ldir):
    ...
    Lmrpm = int(((((LinVel-((AngVel*b)/2))/r)*180/math.pi)/6)*GearRatio) # Left Motor Rpm
    self.processImage.variables.RightMotor.RxPDO_Mapping_1.Target_velocity.set(Rdir*Rmrpm)
    self.processImage.variables.LeftMotor.RxPDO_Mapping_1.Target_velocity.set(Ldir*Lmrpm)
    return Rmrpm,Lmrpm
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
def WheelStart(self): 
...
def LiftUp(self,TrajType= None):
    if TrajType == 1:
        print("------------------ Lift Up ------------------")
        TrajTime = 2 # 2 second trajectory profile
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_velocity.set(2025)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_acceleration.set(3037)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_deceleration.set(3037)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Target_position.set(-184320)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(128)
        time.sleep(1) # 1 second sleep
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(6)
        time.sleep(1) # 1 second sleep
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(15)
        time.sleep(1) # 1 second sleep
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(63)
        time.sleep(TrajTime + 0.1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(6)
        
    elif TrajType == 2:
        print("------------------ Lift Up ------------------")
        TrajTime = 2.5 # 2.5 second trajectory profile
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_velocity.set(1620)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_acceleration.set(1944)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_deceleration.set(1944)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Target_position.set(-184320)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(128)
        ...
        print("------------------ Lift Up ------------------")
        TrajTime = 3 # 3 second trajectory profile
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_velocity.set(1350)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_acceleration.set(1350)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_deceleration.set(1350)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Target_position.set(-184320)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(128)
        ...
        print("------------------ Lift Up ------------------")
        TrajTime = 6 # 6 second trajectory profile
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_velocity.set(675)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_acceleration.set(337)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_deceleration.set(337)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Target_position.set(-184320)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(128)
...
def LiftDown(self,TrajType= None):
    if TrajType == 1:
        ...
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Target_position.set(0)
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(128)
        time.sleep(1) # 1 second sleep
        self.processImage.variables.LiftMotor.Receive_PDO1_mapping.Controlword.set(6)
        time.sleep(1) # 1 second sleep
        ...
        print("------------------ Lift Down ----------------")
        TrajTime = 4 # 6 second trajectory profile
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_velocity.set(675)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_acceleration.set(337)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Profile_deceleration.set(337)
        self.processImage.variables.LiftMotor.Receive_PDO4_mapping.Quick_stop_deceleration.set(10000)
...
# sudo python3 EcDemoApp.py --mode 1 -f ENI.xml --link "sockraw enp4s0 1" -b 4000 -t 0 -v 3
# mode -> 1 Master
# f -> ENI EtherCAT Network Information
# lin -> enp4s0 EtherCAT Socket
# b -> Cycle Time
# t -> Duration Time
# LiftMotor    
