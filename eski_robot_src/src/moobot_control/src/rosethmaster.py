...
# --------------------------------------------------------------------------------------------------
    def speedup(self,Twist):
            ...
            # time.sleep(0.0001)
            print(f'----------------- Right Trajectory Velocity (Rpm) -> {Rdir*RM} ------------------\n')
            # print(f'------------------- Right Actual Velocity (Rpm) -> {GetVelActual(self)[0]} --------------\n')
            print(f'----------------- Left Trajectory Velocity (Rpm) -> {Ldir*LM} -------------------\n')
            # print(f'------------------- Left Actual Velocity (Rpm) -> {GetVelActual(self)[1]} --------------\n')
            # time.sleep(.001)
            # --------------- Actual Velocity Publisher ------
            r = .105   # Wheel Radius m
            b = .4775  # AGV Wheel Offset m
            GearRatio = 24.685 # Const Value
            R_actual,L_actual= GetVelActual(self)  # rpm actual
            WR_actual= (((R_actual/GearRatio)*math.pi/180)*6)  # rad/s actual right 
            VR_actual=WR_actual*r # m/s actual right 
            WL_actual= (((L_actual/GearRatio)*math.pi/180)*6)  # rad/s actual left 
    ...
    def runDemo(self, TrajType=None):
        endTimeInMs = 0
        global speed_sub, drive_manual, reset_done, cmd_vel_msg, twist_pub, cancel_pub, moobot_status_pub_s_plc, emg_from_c_plc_sub, scanner_area_sub, scanner_area_pub, scanner_status_pub, moobot_sensor_status_sub_s_plc
...
def MotorInit(self):
    ...
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_acceleration.set(50)
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_deceleration.set(100)
...
# -------------------------------------------------------------------------------------------------
    ...
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_acceleration.set(50)
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_deceleration.set(100)
...
def WheelVelocityRight(self, Rdir, LinVel, AngVel):
    ...
    self.processImage.variables.RightMotor.RxPDO_Mapping_1.Target_velocity.set(
        Rdir*Rmrpm)
    return Rmrpm
# --------------------------------------------------------------------------------------------------
...
# / 3 -> Max limit  // Scanner Area 4 (For lift area
...
def CheckMessage(InputData):
    ...
    global old_emg_situation, old_stop, old_reset_done, drive_manual
    ...
    # #***************
    # # fs warning 1
    # if XTDI3_input_message[2] == '0':
    #     scanner_status_msg.fs_w1 = 1
    # else:
    #     scanner_status_msg.fs_w1 = 0
    # #***************
    # # fs warning 2
    # if XTDI3_input_message[3] == '0':
    #     scanner_status_msg.fs_w2 = 1
    # else:
    #     scanner_status_msg.fs_w2 = 0
    ...
    if current_stop != old_stop:
        if current_stop:
            ...
            moobot_status_pub_s_plc.publish(moobot_status_msg)
    # if old_agv_brake != agv_brake:
    #     send_brake(agv_brake)
    #     if agv_brake:
    #         stop_agv(current_mode)
    #     else:
    #         agv_brake = False
...
def change_mode(mode):
    if mode=="MANUAL":
        print("Manual drive is ready")
        rospy.Subscriber("/cmd_vel", Twist, EcDemoApp.speedup)
...
#     Rdir = 1
#     Ldir = -1
#     RM = WheelVelocityRight(self, Rdir, AgvLinearVel, AgvAngVel)
#     LM = WheelVelocityLeft(self, Ldir, AgvLinearVel, AgvAngVel)
#     print('***********************************************************************************')
#     print(f'----------------- Right Trajectory Velocity (Rpm) -> {Rdir*RM} ------------------\n')
#     print(f'----------------- Left Trajectory Velocity (Rpm) -> {Ldir*LM} -------------------\n')
# --------------------------------------------------------------------------------------------------
