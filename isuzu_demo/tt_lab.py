...
DOLLY_1 = 3      
CIKIS_DOLLY_1 = 4 # 
GIRIS_KAMYON_ALANI = 5 #1.5907
KAMYON_ALANI = 6 #1.5907
CIKIS_KAMYON = 7     #1.5915
GIRIS_BOS_DEPO_ALANI = 8
BOS_DEPO_ALANI = 9 #DOLLY_1 İLE AYNI 
CIKIS_BOS_DEPO = 10
KAMYON_ALANI_DOLLY2_GIRIS  = 11
KAMYON_ALANI_DOLLY2  = 12     # KAMYON_ALANI İLE AYNI
GIRIS_SON_DEPO= 13
SON_DEPO = 14 # DOLLY_1 İLE AYNI 
# FINISH = 12 
...
DOLLY_1_GO = 43
CIKIS_DOLLY_1_GO = 44
GIRIS_KAMYON_ALANI_GO = 45
KAMYON_ALANI_GO = 46
CIKIS_KAMYON_GO = 47
GIRIS_BOS_DEPO_ALANI_GO = 48
BOS_DEPO_ALANI_GO = 49
CIKIS_BOS_DEPO_GO = 50
KAMYON_ALANI_DOLLY2_GIRIS_GO = 51
KAMYON_ALANI_DOLLY2_GO = 52
GIRIS_SON_DEPO_GO=53
SON_DEPO_GO = 54
...
GIRIS_KAMYON_ALANI_WAIT = 75
KAMYON_ALANI_WAIT = 76
CIKIS_KAMYON_WAIT = 77
GIRIS_BOS_DEPO_ALANI_WAIT = 78
BOS_DEPO_ALANI_WAIT = 79
CIKIS_BOS_DEPO_WAIT = 80
KAMYON_ALANI_DOLLY2_GIRIS_WAIT = 81
KAMYON_ALANI_DOLLY2_WAIT = 82
GIRIS_SON_DEPO_WAIT=83
SON_DEPO_WAIT = 84
...
a_yaml_file = open(
    "/home/rnd/moobot_ws/src/moobot_navigation/isuzu_mayis.rviz")
...
for station in stations:
    ...
    if number == 1:
        ...
        new_station = Station(GIRIS_KAMYON_ALANI,
                              station_x, station_y, station_z)
        stations_list.append(new_station)
    elif number == 6:
        ...
        new_station = Station(GIRIS_BOS_DEPO_ALANI,
                              station_x, station_y, station_z)
        stations_list.append(new_station)
    elif number == 9:
        ...
        new_station = Station(KAMYON_ALANI_DOLLY2_GIRIS,
                              station_x, station_y, station_z)
        stations_list.append(new_station)
    elif number == 12:
        ...
        new_station = Station(GIRIS_SON_DEPO,
                              station_x, station_y, station_z)
        stations_list.append(new_station)
    elif number == 14:
...
def send_goal_simple(x_goal, y_goal, station_num, current_station):
    global current_pos
    global goal_sended
    goal_sended = True
    ...
    # q = euler_to_quaternion(0.0, 0.0, orientation)
    goal.target_pose.pose = Pose(
        Point(x_goal, y_goal, 0.000), Quaternion(0, 0, 0, 1))
    move_base_client.send_goal(goal)
    success = move_base_client.wait_for_result()
    state = move_base_client.get_state()
    if state != 3:
...
def send_station_simple(goal_station, current_station):
    global current_pos
    # current_pos = current_pos + 8
    for s in stations_list:
        if s.number == goal_station:         
            ...
            # orientation = orientations[s.number-1]
            print("Set goal on station ", s.number)
            send_goal_simple(x_goal, y_goal, goal_station, current_station)
            break
...
def listen_actual_vel(data):
    global AgvLinearVel,AgvAngVel
    AgvLinearVel= data.linear.x
    AgvAngVel= data.angular.z

def simple_goal_check(data):
    global status_info
...
duz_git_pub = rospy.Publisher("/move_base_simple/goal", PoseStamped, queue_size=1)
duz_git_msg = PoseStamped()

actual_cel = rospy.Subscriber("/cmd_vel_act", Twist, listen_actual_vel)
# simple_goal_check_sub = rospy.Subscriber("/move_base/result", MoveBaseActionResult, simple_goal_check)
...
'}\""""
                ...
                #                     - {name: '', value: ''}
                #                     doubles:
                #                     - {name: 'xy_goal_tolerance', value: 0.08}
...
# 2. İSTASYON - giriş1
                ...
                tolerance_hassas = """rosservice call /move_base/DWAPlannerROS/set_parameters \"config:
                                    ...
                                    - {name: '', value: ''}
                                    doubles:
                                    - {name: 'xy_goal_tolerance', value: 0.15}
...
# 3. İSTASYONA - dolly_load
                ...
                time.sleep(1.0)
                once_run=True
                # hareket_kontrol = False
                current_pos = DOLLY_1_GO

            elif current_pos == DOLLY_1_GO:
                if goal_sended == False:
                    next_station = DOLLY_1
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()
                    # duz_git_msg.header.frame_id = "base_link"
                    # duz_git_msg.pose.position.x = 2.0
                    # duz_git_msg.pose.orientation.z = 0.0
                    # duz_git_msg.pose.orientation.w = 1.0
                    # if once_run == True:
                    #     duz_git_pub.publish(duz_git_msg)
                    #     once_run= False
                # goal_sended = True
                # success = move_base_client.wait_for_result()
                # state = move_base_client.get_state()
                # if state ==1:
                #     print("Harekete başladı")
                #     once_run= False
                # if state != 3 and not once_run:
                #     print(state)
                # if success and state == 3 and not once_run:  # goal a gidebilmiş
                #     current_pos = DOLLY_1
                #     print(current_pos)
                #     goal_sended = False


                    # if abs(AgvLinearVel)> 0.05 and abs(AgvAngVel) > 0.01:
                    #     hareket_kontrol = True
                    # if hareket_kontrol== True and abs(AgvLinearVel)==0 and abs(AgvAngVel)==0:
                    #     current_pos = DOLLY_1
...
# 4. İSTASYONA - giriş (2 ile aynı)
                ...
                tolerance_hassas_olmayan = """rosservice call /move_base/DWAPlannerROS/set_parameters \"config:
                                            ...
                                            doubles:
                                            - {name: 'xy_goal_tolerance', value: 0.08}
                                            - {name: 'yaw_goal_tolerance', value: 0.08}
                                            groups:
                                            - {name: '', state: false, id: 0, parent: 0}\""""
                ...
                if goal_sended == False:
                    current_pos = GIRIS_KAMYON_ALANI_WAIT
# 3. İSTASYONA - dolly_load
            elif current_pos == GIRIS_KAMYON_ALANI_WAIT:
                print("DOLLY_1'a girmek için 5 saniye bekleniyor...")
                time.sleep(1.0)
                current_pos = GIRIS_KAMYON_ALANI_GO

            elif current_pos == GIRIS_KAMYON_ALANI_GO:
                if goal_sended == False:
                    next_station = GIRIS_KAMYON_ALANI
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()

            elif current_pos == GIRIS_KAMYON_ALANI:
                print("CIKIS_DOLLY_1'a geldim. ")
                if goal_sended == False:
...
# 7. İSTASYONA giriş2 (5 ile aynı)
                ...
                print("CIKIS_KAMYON'a gitme için 5 saniye bekleniyor... Scanner ++ & Tolerance-> 1.0")
                scanner_normal_area_front = """rosservice call /front_laser_scanner/front_laser_scanner/set_parameters "config:
                                            bools:
                                            - {name: '', value: false}
                                            ints:
                ...
                # os.system(tolerance_hassas)
                if goal_sended == False:
                    current_pos = GIRIS_BOS_DEPO_ALANI_WAIT

            elif current_pos == GIRIS_BOS_DEPO_ALANI_WAIT:
                print(" BOS_DEPO_ALANI'a gitme için 1 saniye bekleniyor...")
                time.sleep(0.0)
                current_pos = GIRIS_BOS_DEPO_ALANI_GO

            elif current_pos == GIRIS_BOS_DEPO_ALANI_GO:
                if goal_sended == False:
                    next_station = GIRIS_BOS_DEPO_ALANI
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()

            elif current_pos == GIRIS_BOS_DEPO_ALANI:
...
# 4. İSTASYONA - giriş (2 ile aynı)
                ...
                print("kamyon_dolly'a gitme için 3 saniye bekleniyor... Tolerance-> 1.0")
                # tolerance_hassas_olmayan = """rosservice call /move_base/DWAPlannerROS/set_parameters \"config:
                #                     bools:
                #                     - {name: '', value: false}
                #                     ints:
                ...
                # os.system(tolerance_hassas)         
                if goal_sended == False:
                    current_pos = KAMYON_ALANI_DOLLY2_GIRIS_WAIT

# 2. İSTASYON - giriş1
            elif current_pos == KAMYON_ALANI_DOLLY2_GIRIS_WAIT:
                print("KAMYON_ALANI_DOLLY2_GIRIS'a gitme için 0.5 saniye bekleniyor...")
                time.sleep(0.5)
                current_pos = KAMYON_ALANI_DOLLY2_GIRIS_GO

            elif current_pos == KAMYON_ALANI_DOLLY2_GIRIS_GO:
                if goal_sended == False:
                    next_station = KAMYON_ALANI_DOLLY2_GIRIS
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()

            elif current_pos == KAMYON_ALANI_DOLLY2_GIRIS:
...
# 3. İSTASYONA - dolly_load
                ...
                print("DOLLY_2'a girmek için 2 saniye bekleniyor...")
                # msg.data = 1
                # scanner_area_change_pub.publish(msg)  # ALAN 1'E GEÇİŞ PAYLAŞILDI                
                time.sleep(1.0)
                once_run=True
                ...
                if goal_sended == False:
                    next_station = KAMYON_ALANI_DOLLY2
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()
                    # duz_git_msg.header.frame_id = "base_link"
                    # duz_git_msg.pose.position.x = 1.9
                    # duz_git_msg.pose.orientation.z = 0
                    # duz_git_msg.pose.orientation.w = 1
                    # if once_run == True:
                    #     duz_git_pub.publish(duz_git_msg)
                    #     once_run=False
                    # if abs(AgvLinearVel)> 0.05 and abs(AgvAngVel) > 0.01:
                ...
                if lift_status_val != LIFT_UP:
                    ...
                    time.sleep(1.0)
                    if goal_sended == False:
                        current_pos = GIRIS_SON_DEPO_WAIT

            elif current_pos == GIRIS_SON_DEPO_WAIT:
                print("GIRIS_SON_DEPO'a gitme için 0.5 saniye bekleniyor...")
                tolerance_hassas = """rosservice call /move_base/DWAPlannerROS/set_parameters \"config:
                                    bools:
                                    - {name: '', value: false}
                                    ints:
                ...
                os.system(footprint_command_local)                                  
                time.sleep(0.5)
                current_pos = GIRIS_SON_DEPO_GO

            elif current_pos == GIRIS_SON_DEPO_GO:
                if goal_sended == False:
                    next_station = GIRIS_SON_DEPO
                    thread_send_station = Thread(
                        target=send_station, args=[next_station, current_pos], daemon=True)
                    thread_send_station.start()

            elif current_pos == GIRIS_SON_DEPO:
                print("GIRIS_SON_DEPO'a geldim. ")
                if goal_sended == False:
