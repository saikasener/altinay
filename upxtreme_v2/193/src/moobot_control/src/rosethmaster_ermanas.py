...
# --------------------------------------------------------------------------------------------------
    ...
    def runDemo(self, TrajType=None):
        endTimeInMs = 0
        global speed_sub, drive_manual, reset_done, cmd_vel_msg, twist_pub, cancel_pub, moobot_status_pub_s_plc
        global emg_from_c_plc_sub, scanner_area_sub, scanner_area_pub, scanner_status_pub, moobot_sensor_status_sub_s_plc
...
def MotorInit(self):
    ...
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_acceleration.set(500)
...
# -------------------------------------------------------------------------------------------------
    ...
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_acceleration.set(500)
...
def CheckMessage(InputData):
    ...
    global old_emg_situation, old_stop, old_reset_done, drive_manual, rate
    ...
    if reset_done != old_reset_done:
        if reset_done:
            ...
            # drive_manual = True
            # print("Manual drive is ready after reset")
            # rospy.Subscriber("/cmd_vel", Twist, self.speedup)
            change_mode(MANUAL)

    # if not emg_situation:
    #     if reset_done or LOGIC_result[2] == '1':
    #         manual_drive(self)
...
#     print("Manual drive is ready after reset")
#     rospy.Subscriber("/cmd_vel", Twist, self.speedup, queue_size=2)
#     # rate.sleep()
