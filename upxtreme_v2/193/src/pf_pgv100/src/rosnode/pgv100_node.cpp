...
int main(int argc, char **argv)
{
    ...
    */
    ...
    while (ros::ok())
    {
        ...
        pf_pgv100::pgv_scan_data pgv_msg;
        pgv_msg.header.stamp = ros::Time::now();
        pgv_msg.header.frame_id = "pgv_link"; 

        pgv_msg.angle = agv_ang_des - cal_err_ang;          // degree
        pgv_msg.x_pos = (agv_x_pos_des - cal_err_x) / 10.0; // mm
        pgv_msg.y_pos = (agv_y_pos_des - cal_err_y) / 10.0; // mm
        pgv_msg.direction = selected_dir;
        pgv_msg.color_lane_count = agv_c_lane_count_des;
        pgv_msg.no_color_lane = agv_no_color_lane_des;
        pgv_msg.no_pos = agv_no_pos_des;
        pgv_msg.tag_detected = tag_detected_des;

        pgv100_pub.publish(pgv_msg);
