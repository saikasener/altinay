...
# --------------------------------------------------------------------------------------------------
    ...
    def change_lift_status(self,lift_status_msg):
        if lift_status_msg.up_lift:
            LiftUp(self)
            lift_status_msg2.data=LIFT_UP
        elif lift_status_msg.down_lift:
            LiftDown(self)
            lift_status_msg2.data=LIFT_DOWN
        else:
            lift_status_msg2.data=LIFT_UNKNOWN
        lift_status_pub.publish(lift_status_msg2)

    def runDemo(self, TrajType=None):
        endTimeInMs = 0
        global reset_done, cmd_vel_msg,  lift_status_msg,lift_status_msg2 , out_msg_1
        global twist_pub,cancel_pub,moobot_status_pub_s_plc, cmd_vel_act_pub, lift_status_pub, scanner_area_pub,scanner_status_pub
        global emg_from_c_plc_sub, scanner_area_sub, moobot_sensor_status_sub_s_plc,twist_sub
        ...
        cmd_vel_act_pub = rospy.Publisher("/cmd_vel_act", Twist, queue_size=1)
        ...
        lift_status_sub = rospy.Subscriber("/change_lift_status", lift_status, self.change_lift_status)
        lift_status_pub = rospy.Publisher("/lift_status", Int16, queue_size=2)
        ...
        while not rospy.is_shutdown():  # While True Until Shutdown Node
            ...
            GetVelActual(self)  # rpm actual --> return RVelAct, LVelAct
            CheckMessage(self,InputPDOByteToBit(GetPlcPdoInput(self)))
            SystemWorksBit(self,out_msg_1)
...
# --------------------------------------------------------------------------------------------------
    def startDemo(self):
        ...
        try:
            ...
            dwRes = self.oEcWrapper.SetLicenseKey("C2953AD5-9FEC28ED-941FB867")
...
def MotorInit(self):
    ...
    self.processImage.variables.LeftMotor.RxPDO_Mapping_3.Profile_deceleration.set(200)
    ...
    self.processImage.variables.RightMotor.RxPDO_Mapping_3.Profile_deceleration.set(200)
...
def GetVelActual(self):
    ...
    b = .400  # AGV Wheel Offset m
    GearRatio = 24.685
    WR_actual= (((RVelAct/GearRatio)*math.pi/180)*6)  # rad/s actual right 
    VR_actual=WR_actual*r # m/s actual right 
    WL_actual= (((LVelAct/GearRatio)*math.pi/180)*6)  # rad/s actual left 
...
def SystemWorksBit(self, out_msg_0):
    self.processImage.variables.SafePLC.Out.Out_Dataset1.set(out_msg_1)
    # print(self.processImage.variables.SafePLC.Out.Out_Dataset1.get())

# def SystemWorksBit2(self, out_msg_1):
#     self.processImage.variables.SafePLC.Out.Out_Dataset2.set(out_msg_1)
#     # print(self.processImage.variables.SafePLC.Out.Out_Dataset1.get()
...
def CheckMessage(self,InputData):
    ...
    XTDI3_input_message = reverse(InputData[0]) #bitlist(bytes([IB0])).bin()
    XTIO1_input_message = reverse(InputData[1]) #bitlist(bytes([IB1])).bin()
    XTIO1_out_message = reverse(InputData[2]) #bitlist(bytes([IB2])).bin()
    LOGIC_result = reverse(InputData[5]) #bitlist(bytes([IB8])).bin()
    ...
    if old_emg_situation != emg_situation:
        if emg_situation:
            moobot_status_msg.emergency = True
            moobot_status_pub_s_plc.publish(moobot_status_msg)
            stop_agv(self,current_mode)
        else:
            moobot_status_msg.emergency = False
            moobot_status_pub_s_plc.publish(moobot_status_msg)
...
def change_mode(mode):
    ...
    moobot_status_pub_s_plc.publish(moobot_status_msg)
# --------------------------------------------------------------------------------------------------
def stop_agv(self,current_mode):
...
def change_area(area_msg):
    ...
    #         if i != 4:
    #             out_msg_1[i] = 0
    scanner_area_narow = area_msg.data
    if  scanner_area_narow==1:
        out_msg_1[3] = 1 #dar alan
    else:
        out_msg_1[3] = 0 #normal alan
...
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
def LiftHome_eski(self):
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(6)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(7)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(31)
    #---------------- Go Go GO Park Pose ----------------
#    while True:
#        if(LiftStatus(self)==5175):
#            break
    time.sleep(6)  # Revision
    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_velocity.set(2000)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_acceleration.set(2000)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(2000)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Target_position.set(40000)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(1)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(63)
    time.sleep(2)
    return "Home Done"
    
# --------------------------------------------------------------------------------------------------
def LiftUp_eski(self):
        print("------------------ Lift Up  ------------------")        
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_velocity.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_acceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Target_position.set(-120320) # 144320
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(0)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(63)
        time.sleep(3.5)
        print("------------------ Lift Up Done ------------------")        
# --------------------------------------------------------------------------------------------------          
def LiftDown_eski(self,TrajType= None):
        print("------------------ Lift Down ----------------")
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_velocity.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_acceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(1350)
...
#        if(LiftStatus(self)==5175):
#            break
    time.sleep(9)  # Revision
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_velocity.set(2000)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_acceleration.set(2000)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(2000)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Target_position.set(50000)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(1)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
#    self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(63)
...
def LiftUp(self):
        ...
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Target_position.set(-140000) # 144320
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(0)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
        ...
        # print("------------------ Lift Up Done ------------------")
# --------------------------------------------------------------------------------------------------
def LiftDown(self,TrajType= None):
        print("------------------ Lift Down ----------------")
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_velocity.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_acceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Profile_deceleration.set(1350)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_3.Quick_stop_deceleration.set(5000)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Target_position.set(0)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Modes_of_operation.set(1)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(0)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(6)
        self.processImage.variables.LiftMotor.RxPDO_Mapping_1.Controlword.set(15)
...
# --------------------------------------- PLC PDO Function -----------------------------------------
# --------------------------------------------------------------------------------------------------
# --------------------------------------- INPUT ~ OUTPUT PDO Bit Mapping ---------------------------
# --------------------------------------- INPUT PDO CONFIG Get Config ------------------------------
# /// IB0 XTDI 3 -> 8 Bit  Input ///
# XTDI_3_IN
# / 0 -> Right Front Emg Button // E-Stop 1. kontak
# / 1 -> Auto Mode Switch  // Mode switch auto contact
# / 2 -> Manuel Mode Switch // Mode switch manual contact
# / 3 -> Right Back Emg Button // Start button
# / 4 -> Warning 2 Scanner // Front Scanner Warning 2
# / 5 -> Warning 1 Scanner // Front Scanner Warning 1
# / 6 -> Front Scanner FS Protection // Front Scanner OSSD 1
# / 7 -> Back Scanner  BS Protection // Back Scanner OSSD 1
# ---------------------------------------
# /// IB1 XTIO 1 -> 8 Bit Input ///
# XTIO_1_IN
# / 0 -> Main Emergency // E-Stop 2. kontak
# / 1 -> Reset Button //  Reset button
# / 2 -> Break Release Button // Single channel ??
# / 3 -> Load Button  // Single channel ??
# / 4 -> Null Space // Back Scanner Warning 2
# / 5 -> Left Front Emergency // Back Scanner Warning 1
# / 6 -> Back Scanner  BS Protection // Back Scanner OSSD 2
# / 7 -> Front Scanner FS Protection // Front Scanner OSSD 2
# ---------------------------------------
# /// IB2 XTIO 1 -> 8 Bit Output///-->
# XTIO_1_OUT
# / 0 -> Low Speed Area // FS_Field1 -> A1
# / 1 -> High Speed Area // FS_Field2 -> A2
# / 2 -> Narrow Place Area // BS_Field1 -> A1
# / 3 -> Max Limit Area //  BS_Field2 -> A2
# / 4 -> Null Space
# / 5 -> Null Space
# / 6 -> Null Space
# / 7 -> Null Space
# ---------------------------------------
# /// IB3 XTIO 2 -> 8 Bit Input///
# XTIO_2_IN
# / 0 -> SSP Works
# / 1 -> Left Back Emg
# / 2 -> Lift Down
...
# / 7 -> SSP MEM Switch
# ---------------------------------------
# /// IB4 XTIO 2 -> 8 Bit Output/// -->
# XTIO_2_OUT
# / 0 -> Null Space // Brake
# / 1 -> Null Space // Res_Req
# / 2 -> Scanner Sleep Mode // Yellow Lamb
# / 3 -> STO / SBC
# / 4 -> Null Space
# / 5 -> Null Space
# / 6 -> Null Space
...
# ---------------------------------------
# /// IB8 Logic Result ///
# / 0 -> General EMG     "Safe State and Stop All Navigation Comment Wait Until Reset Done"
# / 1 -> PC Must Reset   "Wake up reset and run navigation, repeat goal id"
# / 2 -> Manuel Mode     "Run system via UI or Joystick"
# / 3 -> Auto Mode       "Run system via Navigation"
# / 4 -> Reset Done      "Reset Done Check, repeat goal id and GO GO GO!"
# / 5 -> Release Brake   "Open Break and Push AGV By Hand"
# / 6 -> SSP Error       "EMG"
# / 7 -> Protection Zone EMG    "EMG"
# /// LOGIC->5 : Pallet Loaded Button
# ---------------------------------------
# /// IB9 Null Space Byte ///
# --------------------------------------- OUTPUT PDO CONFIG Set Config -----------------------------
# ---------------------------------------
# /// QB0  ///
# OUT_MSG_0
# / 0 -> IMU Error
# / 1 -> Scanner Data Error
# / 2 -> Driver Error  // C-PLC e-stop butonu
# / 3 -> BMS Error
# / 4 -> Lift Error  // Conveyor error
# / 5 -> PGV Error
# / 6 -> Reset Done
# / 7 -> System Works *******
# ---------------------------------------
# /// QB1  ///
# OUT_MSG_1
# / 0 -> Low Speed Area // Scanner Area 1 (For conveyor area)
# / 1 -> High Speed Area // Scanner Area 2 (For robot area)
# / 2 -> Narrow Place Area // Scanner Area 3 (For high speed area)
# / 3 -> Dolly area  // en küçük area= 1 --> 0 = normal area
# / 4 -> Just Brake Signal  // PC to PLC Brake Signal
# / 5 -> UI Auto Mode //  Share AGV mode from UI
# / 6 -> UI Manual Mode // Share AGV mode from UI
# / 7 -> PC EMG
# ---------------------------------------
# /// QB2 Null Space Byte ///
# ---------------------------------------
