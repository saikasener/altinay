...
// PLC'den çok hızlı şekilde sürekli data alıyoruz.
// Sistemin geri kalanına veriyi sadece değiştiğinde yollamak gerekiyor
bool old_stop = false;
bool current_stop;
int old_mode;
int error_status;
...
bool old_agv_brake = true;
bool agv_brake = false;
...
// Burada çok güzel şeyler planladık.
// Sistemin bi kısmında arıza olursa PLC'ye söyleyelim izin verdiğinde sadece o kısmı resetleyelim istedik :(
void reset_system()
{
   switch (error_status)
   {
...
/// OUT_MSG_1->4 : PC to PLC Brake Signal
/// OUT_MSG_1->5 :
/// OUT_MSG_1->6 :
/// OUT_MSG_1->7 :
/////////////////////////////////////////
...
void change_area(const std_msgs::Int16 &area_msg) // sens input scanner area pins to plc
{
...
void send_brake(bool brake_signal) // sends brake signal
{
   if (brake_signal)
      out_msg_1[4] = 1;
   else
...
void check_area_info(std::string XTIO1_out_message) // checks scanner area according to A1 and A2 pins
{
...
// A general function to check messages from plc
void checkMessage(std::string XTIO1_input_message, std::string XTDI3_input_message, std::string XTIO3_out_message, std::string LOGIC_result, std::string XTIO1_out_message)
{
   ...
   if (LOGIC_result.at(7) == '1') // Emergency signal comes from plc
   {
      current_stop = true;
      emg_situation = true;
   }
   ...
   agv_brake = (ultrasonic_status1_2 || ultrasonic_status3_4) && current_scanner_area_type != 4;
   // agv_brake = ultrasonic_status1_2 || ultrasonic_status3_4;
...
// Ethernet haberleşme fonksiyonu
...
// Bu kısım agv kontrolcüsü dışında bir kontrolcüden kontrol edildiğinde gerekli
...
int ultrasonic_status1_2_counter, ultrasonic_status3_4_counter;
void check_ultrasonic_status1_2(const moobot_msgs::ultrasonic_status &msg)
{
   if (msg.S_Protection == true)
      ultrasonic_status1_2_counter++;
   else
      ultrasonic_status1_2_counter = 0;
   if (ultrasonic_status1_2_counter == 10)
      ultrasonic_status1_2 = true;
   else
      ultrasonic_status1_2 = false;
}
void check_ultrasonic_status3_4(const moobot_msgs::ultrasonic_status &msg)
{
   if (msg.S_Protection == true)
      ultrasonic_status3_4_counter++;
   else
      ultrasonic_status3_4_counter = 0;
   if (ultrasonic_status3_4_counter == 10)
      ultrasonic_status3_4 = true;
   else
      ultrasonic_status3_4 = false;
}
      ...
      if (ec_config_init(FALSE) > 0)
      {
         ...
         if (ec_slave[0].state == EC_STATE_OPERATIONAL)
         {
            ...
            while (!stop_program)
            {
               ...
               if (wkc >= expectedWKC)
               {
                  // PLC input ve output mesajlarını yazdırmak için
