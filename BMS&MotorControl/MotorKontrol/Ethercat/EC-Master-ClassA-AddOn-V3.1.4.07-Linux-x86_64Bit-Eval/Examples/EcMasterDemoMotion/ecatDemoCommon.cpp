...
EC_T_VOID FlushLogBuffer(T_DEMO_THREAD_PARAM* pDemoThreadParam, EC_T_CHAR* szLogBuffer)
{
...
{
    emEthDbgMsg(pDemoThreadParam->dwInstanceId, byEthTypeByte0, byEthTypeByte1, szMsg);
}
...
EC_T_BOOL PrintSlaveInfos(T_DEMO_THREAD_PARAM* pDemoThreadParam)
{
    ...
    EC_T_CFG_SLAVE_INFO oCfgSlaveInfo;
        ...
        dwRes = emGetBusSlaveInfo(pDemoThreadParam->dwInstanceId, EC_FALSE, wAutoIncAddress, &oBusSlaveInfo);
        ...
        dwRes = emGetCfgSlaveInfo(pDemoThreadParam->dwInstanceId, EC_TRUE, oBusSlaveInfo.wStationAddress, &oCfgSlaveInfo);
        if (EC_E_NOERROR != dwRes)
        {
            continue;
        }
...
EC_T_VOID PrintCfgSlavesInfo(T_DEMO_THREAD_PARAM* pDemoThreadParam)
{
    ...
    for (wAutoIncAddress = 0; wAutoIncAddress != 1; wAutoIncAddress--)
    {
        ...
        dwRes = emGetCfgSlaveInfo(pDemoThreadParam->dwInstanceId, EC_FALSE, wAutoIncAddress, &oCfgSlaveInfo);
        if (EC_E_NOERROR != dwRes)
        {
            break;
        }
        ...
        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "Slave is optional  : %s\n", ((oCfgSlaveInfo.dwHCGroupIdx == 0) ? "Yes" : "No")));
    ...
    ,EC_T_WORD*           pwPhysAddr)         /**< [out]  Physical Address of slave */
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConnectedSlaves(pDemoThreadParam->dwInstanceId); dwSlaveIdx++)
    {
    ...
    ,EC_T_WORD*           pwPhysAddr)         /**< [out]  Physical Address of slave */
{
    ...
    for (dwSlaveIdx = 0; dwSlaveIdx < emGetNumConfiguredSlaves(pDemoThreadParam->dwInstanceId); dwSlaveIdx++)
    {
        EC_T_WORD           wAutoIncAddress = (EC_T_WORD)(0 - dwSlaveIdx);
        EC_T_CFG_SLAVE_INFO oCfgSlaveInfo;
...
\return Vendor Text.
*/
const EC_T_CHAR* SlaveVendorText(T_eEtherCAT_Vendor EV       /**< [in]   Vendor ID */)
{
...
\return PC Text.
*/
const EC_T_CHAR* SlaveProdCodeText(
...
*/
EC_T_VOID ParseDiagMsg(T_DEMO_THREAD_PARAM* pDemoThreadParam, EC_T_OBJ10F3_DIAGMSG* pDiag)
{
    static
...
#define MAX_OBNAME_LEN          ((EC_T_DWORD)100)
    ...
    dwClientId = pDemoThreadParam->pNotInst->GetClientID();
    ...
    pMbxGetODLTfer = emMbxTferCreate(pDemoThreadParam->dwInstanceId, &MbxTferDesc);
    ...
    pMbxGetObDescTfer = emMbxTferCreate(pDemoThreadParam->dwInstanceId, &MbxTferDesc);
    ...
    pMbxGetEntryDescTfer = emMbxTferCreate(pDemoThreadParam->dwInstanceId, &MbxTferDesc);
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_ALL, dwTimeout);
    ...
    HandleMbxTferReqError(pDemoThreadParam, (EC_T_CHAR*)"CoeReadObjectDictionary: Error in emCoeGetODList(ALL)", dwRes, pMbxGetODLTfer);
    if (EC_E_NOERROR != dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    if (pDemoThreadParam->nVerbose > 1)
    {
    ...
    for (wODListLen = 0, wIndex = 0; wIndex < (pMbxGetODLTfer->MbxData.CoE_ODList.wLen); wIndex++)
    {
        ...
        if (pDemoThreadParam->nVerbose > 1)
        {
            ...
            if (((wIndex + 1) % 10) == 0) FlushLogBuffer(pDemoThreadParam, szLogBuffer);
        }
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_RxPdoMap, dwTimeout);
    if (EC_E_SLAVE_NOT_PRESENT == dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    /* wait until transfer object is available incl. logging error */
    HandleMbxTferReqError(pDemoThreadParam, (EC_T_CHAR*)"CoeReadObjectDictionary: Error in emCoeGetODList(RxPdoMap)", dwRes, pMbxGetODLTfer);
    if (EC_E_NOERROR != dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    if (pDemoThreadParam->nVerbose > 1)
    {
        ...
        FlushLogBuffer(pDemoThreadParam, szLogBuffer);
    }
    /* MbxGetODLTfer done */
    pMbxGetODLTfer->eTferStatus = eMbxTferStatus_Idle;
    ...
    dwRes = emCoeGetODList(pDemoThreadParam->dwInstanceId, pMbxGetODLTfer, dwNodeId, eODListType_TxPdoMap, dwTimeout);
    if (EC_E_SLAVE_NOT_PRESENT == dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    /* wait until transfer object is available incl. logging error */
    HandleMbxTferReqError(pDemoThreadParam, (EC_T_CHAR*)"CoeReadObjectDictionary: Error in emCoeGetODList(TxPdoMap)", dwRes, pMbxGetODLTfer);
    if (EC_E_NOERROR != dwRes)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    for (wIndex = 0; (wIndex < wODListLen) && !*pbStopReading; wIndex++)
    {
        ...
        /* get object description */
        dwRes = emCoeGetObjectDesc(pDemoThreadParam->dwInstanceId, pMbxGetObDescTfer, dwNodeId, pwODList[wIndex], dwTimeout);
        if (EC_E_SLAVE_NOT_PRESENT == dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
        ...
        /* wait until transfer object is available incl. logging error */
        HandleMbxTferReqError(pDemoThreadParam, (EC_T_CHAR*)"CoeReadObjectDictionary: Error in emCoeGetObjectDesc", dwRes, pMbxGetODLTfer);
        if (EC_E_NOERROR != dwRes)
        {
            dwRetVal = dwRes;
            goto Exit;
        ...
        for (wSubIndex = 0; wSubIndex < wSubIndexLimit; wSubIndex++)
        {
            ...
            dwRes = emCoeGetEntryDesc(pDemoThreadParam->dwInstanceId, pMbxGetEntryDescTfer, dwNodeId, pwODList[wIndex], EC_LOBYTE(wSubIndex), byValueInfoType, dwTimeout);
            if (EC_E_SLAVE_NOT_PRESENT == dwRes)
            {
                dwRetVal = dwRes;
                goto Exit;
            ...
            HandleMbxTferReqError(pDemoThreadParam, (EC_T_CHAR*)"CoeReadObjectDictionary: Error in emCoeGetEntryDesc", dwRes, pMbxGetEntryDescTfer);
            ...
            if (bPerformUpload)
            {
                ...
                dwRes = emCoeSdoUpload(
                    pDemoThreadParam->dwInstanceId, dwNodeId, pwODList[wIndex], EC_LOBYTE(wSubIndex),
                    ...
                    default:
                        {
                            ...
                            for (dwIdx = 0; dwIdx < dwUploadBytes; dwIdx++)
                            {
                                ...
                                if ((0 != dwIdx) && (0 == (dwIdx % 32))) FlushLogBuffer(pDemoThreadParam, szLogBuffer);
                            }
                            FlushLogBuffer(pDemoThreadParam, szLogBuffer);
                        } break;
                    } /* switch (pMbxGetEntryDescTfer->MbxData.CoE_EntryDesc.wDataType) */
...
Exit:
    ...
    if (EC_NULL != pMbxGetODLTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwInstanceId, pMbxGetODLTfer);
        pMbxGetODLTfer = EC_NULL;
    }
    if (EC_NULL != pMbxGetObDescTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwInstanceId, pMbxGetObDescTfer);
        pMbxGetObDescTfer = EC_NULL;
    }
    if (EC_NULL != pMbxGetEntryDescTfer)
    {
        emMbxTferDelete(pDemoThreadParam->dwInstanceId, pMbxGetEntryDescTfer);
